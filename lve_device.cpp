...
namespace lve
{
  ...
  void LveDevice::pickPhysicalDevice()
  {
    ...
    for (const auto &device : devices)
    {
      ...
      // Keep track of first suitable device as fallback
      if (bestDevice == VK_NULL_HANDLE && isDeviceSuitable(device))
      {
        bestDevice = device;
      }
    }

    // Fallback if no discrete GPU found
    if (bestDevice != VK_NULL_HANDLE)
    {
      physicalDevice = bestDevice;
      vkGetPhysicalDeviceProperties(physicalDevice, &properties);
      std::cout << "Selected fallback GPU: " << properties.deviceName << std::endl;
      return;
    }
